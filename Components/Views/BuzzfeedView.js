...
export default class BuzzfeedView extends Component {
  constructor(){
    super()
    this.state = {
      buzzfeedArticles: []
  ...
  componentDidMount(){
      axios.get('http://localhost:3001/api/getBuzzfeed').then(response => {
        // console.error("this is the response data:",response.data[0])  
      this.setState({buzzfeedArticles: response.data} )
      }).catch(error => console.warn(error))
    }
  
  
  render() {
  const articlesList = this.state.buzzfeedArticles.map((item,i)=>{
  ...
  return (
      ...
      <View style={{width: '100%', height: 70, backgroundColor: '#1C1C1C',display:'flex',flexDirection:'row',justifyContent:"space-between",alignItems:'center'}}>
                <Icon name="arrow-back" style={{color:'#F9F9F9',width:10,marginLeft:15}}/>
                  <Image source={buzzfeedLogo} style={{height:30,width:30,marginRight:15}}/>
            </View>
